#!/bin/bash
# =============================================================================
# AppImage Builder for SDR++Brown (ARM64/aarch64 version)
# =============================================================================
# Builds a portable AppImage that bundles all runtime dependencies.
# Designed to run inside an Ubuntu 20.04 (focal) Docker container on ARM64.
#
# Usage:
#   ./make_appimage_aarch64.sh              # Build from /root/SDRPlusPlus
#   SRC_DIR=/path ./make_appimage_aarch64.sh # Build from custom source dir
#
# Output: /root/SDRPlusPlusBrown-aarch64.AppImage
# =============================================================================
...
# ---------------------------------------------------------------------------
# 2. Install linuxdeploy (AppImage tool) - ARM64 version
# ---------------------------------------------------------------------------
echo "=== Step 2: Install linuxdeploy (ARM64) ==="
wget -q -O /tmp/linuxdeploy-aarch64.AppImage \
    "https://github.com/linuxdeploy/linuxdeploy/releases/download/continuous/linuxdeploy-aarch64.AppImage" || true
if [ -f /tmp/linuxdeploy-aarch64.AppImage ] && [ -s /tmp/linuxdeploy-aarch64.AppImage ]; then
    chmod +x /tmp/linuxdeploy-aarch64.AppImage
    # linuxdeploy needs FUSE but inside Docker we use --appimage-extract
    /tmp/linuxdeploy-aarch64.AppImage --appimage-extract > /dev/null 2>&1
    mv squashfs-root /linuxdeploy
    ln -sf /linuxdeploy/AppRun /usr/local/bin/linuxdeploy
else
    echo "ERROR: Failed to download linuxdeploy for ARM64"
    exit 1
fi
...
if [ -f "$DESKTOP_FILE" ]; then
    # Fix paths to be relative to AppDir root (linuxdeploy expects usr/ layout)
    # CMake now generates the correct executable name (sdrpp_brown_appimage for AppImage)
    sed -i \
        -e 's|Exec=/usr/bin/sdrpp_brown_appimage|Exec=usr/bin/sdrpp_brown_appimage|' \
        -e 's|Icon=/.*|Icon=sdrpp|' \
        "$DESKTOP_FILE"
...
# Try aarch64 appimagetool first, then arm64
wget -q -O /tmp/appimagetool-aarch64.AppImage \
    "https://github.com/AppImage/AppImageKit/releases/download/continuous/appimagetool-aarch64.AppImage" || true
if [ -f /tmp/appimagetool-aarch64.AppImage ] && [ -s /tmp/appimagetool-aarch64.AppImage ]; then
    chmod +x /tmp/appimagetool-aarch64.AppImage
    /tmp/appimagetool-aarch64.AppImage --appimage-extract > /dev/null 2>&1
    mv squashfs-root /appimagetool 2>/dev/null || true
    /appimagetool/AppRun "$APPDIR" /root/"$APP_NAME-$ARCH.AppImage" 2>&1 || \
    /tmp/appimagetool-aarch64.AppImage "$APPDIR" /root/"$APP_NAME-$ARCH.AppImage" 2>&1 || \
